...
#    `functionName` is a function's name in your module (e.g., `stateVarsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H16/Lab/Harvesting/stateVars/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("stateVars")
  ...
  parameters <- list(
    ...
    stateVars = list(.saveInitialTime = NA)
  )
  ...
  if (exists("stateVarsEvent1", envir = .GlobalEnv)) {
    simOutput <- stateVarsEvent1(mySim)
  } else {
    simOutput <- mySim$stateVarsEvent1(mySim)
  }
  ...
  if (exists("stateVarsEvent2", envir = .GlobalEnv)) {
    simOutput <- stateVarsEvent2(mySim)
  } else {
    simOutput <- mySim$stateVarsEvent2(mySim)
  }
