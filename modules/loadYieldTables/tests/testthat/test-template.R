...
#    `functionName` is a function's name in your module (e.g., `yieldTablesEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H16/Lab/Harvesting/yieldTables/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("yieldTables")
  ...
  parameters <- list(
    ...
    yieldTables = list(.saveInitialTime = NA)
  )
  ...
  if (exists("yieldTablesEvent1", envir = .GlobalEnv)) {
    simOutput <- yieldTablesEvent1(mySim)
  } else {
    simOutput <- mySim$yieldTablesEvent1(mySim)
  }
  ...
  if (exists("yieldTablesEvent2", envir = .GlobalEnv)) {
    simOutput <- yieldTablesEvent2(mySim)
  } else {
    simOutput <- mySim$yieldTablesEvent2(mySim)
  }
