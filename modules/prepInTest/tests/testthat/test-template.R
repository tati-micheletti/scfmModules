...
#    `functionName` is a function's name in your module (e.g., `prepInTestEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/scfmModules/modules/prepInTest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("prepInTest")
  ...
  parameters <- list(
    ...
    prepInTest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("prepInTestEvent1", envir = .GlobalEnv)) {
    simOutput <- prepInTestEvent1(mySim)
  } else {
  ...
  if (exists("prepInTestEvent2", envir = .GlobalEnv)) {
    simOutput <- prepInTestEvent2(mySim)
  } else {
