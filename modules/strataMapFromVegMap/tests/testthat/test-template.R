...
#    `functionName` is a function's name in your module (e.g., `strataMapFromVegMapEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/scfmModules/modules/strataMapFromVegMap/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("strataMapFromVegMap")
  ...
  parameters <- list(
    ...
    strataMapFromVegMap = list(.saveInitialTime = NA)
  )
  ...
  if (exists("strataMapFromVegMapEvent1", envir = .GlobalEnv)) {
    simOutput <- strataMapFromVegMapEvent1(mySim)
  } else {
  ...
  if (exists("strataMapFromVegMapEvent2", envir = .GlobalEnv)) {
    simOutput <- strataMapFromVegMapEvent2(mySim)
  } else {
