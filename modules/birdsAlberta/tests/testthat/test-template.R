...
#    `functionName` is a function's name in your module (e.g., `birdsAlbertaEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/scfmModules/modules/birdsAlberta/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("birdsAlberta")
  path <- list(modulePath = "C:/Users/tmichele/GitHub/scfmModules/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    birdsAlberta = list(.saveInitialTime = NA)
  )
  ...
  if (exists("birdsAlbertaEvent1", envir = .GlobalEnv)) {
    simOutput <- birdsAlbertaEvent1(mySim)
  } else {
  ...
  if (exists("birdsAlbertaEvent2", envir = .GlobalEnv)) {
    simOutput <- birdsAlbertaEvent2(mySim)
  } else {
