...
#    `functionName` is a function's name in your module (e.g., `harvestEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H16/Lab/Hanzlik/harvest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("harvest")
  path <- list(modulePath = "/Users/stevec/Dropbox/Courses/7043H16/Lab/Hanzlik",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    harvest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("harvestEvent1", envir = .GlobalEnv)) {
    simOutput <- harvestEvent1(mySim)
  } else {
    simOutput <- mySim$harvestEvent1(mySim)
  }
  ...
  if (exists("harvestEvent2", envir = .GlobalEnv)) {
    simOutput <- harvestEvent2(mySim)
  } else {
    simOutput <- mySim$harvestEvent2(mySim)
  }
