...
#    `functionName` is a function's name in your module (e.g., `scfmHarvestEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/scfmModules/modules/scfmHarvest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("scfmHarvest")
  ...
  parameters <- list(
    ...
    scfmHarvest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("scfmHarvestEvent1", envir = .GlobalEnv)) {
    simOutput <- scfmHarvestEvent1(mySim)
  } else {
  ...
  if (exists("scfmHarvestEvent2", envir = .GlobalEnv)) {
    simOutput <- scfmHarvestEvent2(mySim)
  } else {
