...
#    `functionName` is a function's name in your module (e.g., `scfmDisturbanceDriverEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/scfmModules/modules/scfmDisturbanceDriver/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("scfmDisturbanceDriver")
  path <- list(modulePath = "/home/tmichele/Documents/GitHub/scfmModules/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    scfmDisturbanceDriver = list(.saveInitialTime = NA)
  )
  ...
  if (exists("scfmDisturbanceDriverEvent1", envir = .GlobalEnv)) {
    simOutput <- scfmDisturbanceDriverEvent1(mySim)
  } else {
  ...
  if (exists("scfmDisturbanceDriverEvent2", envir = .GlobalEnv)) {
    simOutput <- scfmDisturbanceDriverEvent2(mySim)
  } else {
