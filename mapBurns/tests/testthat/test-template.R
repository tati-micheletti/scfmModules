...
#    `functionName` is a function's name in your module (e.g., `mapBurnsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043/H18/Labs/scfmModules/mapBurns/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("mapBurns")
  path <- list(modulePath = "/Users/stevec/Dropbox/Courses/7043/H18/Labs/scfmModules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    mapBurns = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mapBurnsEvent1", envir = .GlobalEnv)) {
    simOutput <- mapBurnsEvent1(mySim)
  } else {
  ...
  if (exists("mapBurnsEvent2", envir = .GlobalEnv)) {
    simOutput <- mapBurnsEvent2(mySim)
  } else {
