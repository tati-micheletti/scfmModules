...
#    `functionName` is a function's name in your module (e.g., `vegMapToStrataMapEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Documents/GitHub/scfmModules/vegMapToStrataMap/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("vegMapToStrataMap")
  path <- list(modulePath = "/Users/stevec/Documents/GitHub/scfmModules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    vegMapToStrataMap = list(.saveInitialTime = NA)
  )
  ...
  if (exists("vegMapToStrataMapEvent1", envir = .GlobalEnv)) {
    simOutput <- vegMapToStrataMapEvent1(mySim)
  } else {
  ...
  if (exists("vegMapToStrataMapEvent2", envir = .GlobalEnv)) {
    simOutput <- vegMapToStrataMapEvent2(mySim)
  } else {
